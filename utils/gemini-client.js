 ...
 * }>}
 */
async function analyzePdfWithGemini(pdfText, apiKeyOverride = null) {
  ...
  const model = (await chrome.storage.local.get(['geminiModel'])).geminiModel || 'gemini-3.5-flash-lite';
...
2. Найти и извлечь значения из раздела "1. ОБЩАЯ ИНФОРМАЦИЯ ПО СОТРУДНИКУ" (проверяя как текстовые строки, так и занесенные поля формы AcroForm):
   - "organization": Организация (например: "ФОТОТЕХ", "ЗСК ГЛАССПРОМ", "ЭкоОкна" и т.д.)
...
`;
  ...
  if (!response.ok) {
    ...
    const errMessage = errorData.error?.message || `Ошибка сервера Gemini (Статус ${response.status})`;
    throw new Error(`Ошибка Gemini API: ${errMessage}`);
  }
  ...
  // Clean JSON markup if any exists
